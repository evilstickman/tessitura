    ...
    query: {
      $allModels: {
        ...
        async delete({ args, model, query: _query }) {
          ...
          const ctx = base as unknown as Record<string, { update: (a: unknown) => Promise<unknown> }>;
          const modelAccessor = model.charAt(0).toLowerCase() + model.slice(1);
          return ctx[modelAccessor].update({
            where,
            data: { deletedAt: new Date() },
          });
        },
        async deleteMany({ args, model, query: _query }) {
          const ctx = base as unknown as Record<string, { updateMany: (a: unknown) => Promise<unknown> }>;
          const modelAccessor = model.charAt(0).toLowerCase() + model.slice(1);
          return ctx[modelAccessor].updateMany({
            ...args,
            data: { deletedAt: new Date() },
          });
        },
